...
public class SeslTextViewReflector {
    ...
    public static boolean semIsTextSelectionProgressing() {
        Method method;
        if (Build.VERSION.SDK_INT >= 29) {
            method = SeslBaseReflector.getDeclaredMethod(mClass, "hidden_semIsTextSelectionProgressing", new Class[0]);
        } else {
            method = Build.VERSION.SDK_INT >= 24 ? SeslBaseReflector.getMethod(mClass, "semIsTextSelectionProgressing", new Class[0]) : null;
        }
        if (method != null) {
    ...
    public static boolean semIsTextViewHovered() {
        Method method;
        if (Build.VERSION.SDK_INT >= 29) {
            method = SeslBaseReflector.getDeclaredMethod(mClass, "hidden_semIsTextViewHovered", new Class[0]);
        } else {
            method = Build.VERSION.SDK_INT >= 24 ? SeslBaseReflector.getMethod(mClass, "semIsTextViewHovered", new Class[0]) : null;
        }
        if (method != null) {
            Object invoke = SeslBaseReflector.invoke(null, method, new Object[0]);
            if (invoke instanceof Boolean) {
