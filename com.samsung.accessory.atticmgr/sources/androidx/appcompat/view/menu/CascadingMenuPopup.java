    ...
    private void showMenu(MenuBuilder menuBuilder) {
        ...
        while (true) {
            if (i4 >= size) {
                z = false;
                break;
            } else if (((MenuItemImpl) menuBuilder.getItem(i4)).isExclusiveCheckable()) {
                z = true;
                break;
            } else {
                i4++;
        ...
        if (z) {
            menuAdapter = new MenuAdapter(menuBuilder, from, this.mOverflowOnly, SUB_MENU_ITEM_LAYOUT);
        } else {
            menuAdapter = new MenuAdapter(menuBuilder, from, this.mOverflowOnly, ITEM_LAYOUT);
        }
        ...
        if (cascadingMenuInfo == null && this.mShowTitle && menuBuilder.getHeaderTitle() != null) {
            FrameLayout frameLayout = (FrameLayout) from.inflate(R.layout.sesl_popup_menu_header_item_layout, (ViewGroup) listView, false);
