...
public abstract class SeslAbsSeekBar extends SeslProgressBar {
    ...
    public SeslAbsSeekBar(Context context, AttributeSet attributeSet, int i, int i2) {
        ...
        if (Build.VERSION.SDK_INT >= 29) {
            saveAttributeDataForStyleable(context, R.styleable.AppCompatSeekBar, attributeSet, obtainStyledAttributes, i, i2);
        }
        ...
        if (obtainStyledAttributes.hasValue(R.styleable.AppCompatSeekBar_seslSeekBarMode)) {
            this.mCurrentMode = obtainStyledAttributes.getInt(R.styleable.AppCompatSeekBar_seslSeekBarMode, 0);
        }
    ...
    private void setThumbPos(int i, Drawable drawable, float f, int i2) {
        ...
        this.mThumbPosX = (i5 + getPaddingLeft()) - (getPaddingLeft() - (intrinsicWidth / 2));
        updateSplitProgress();
    }
        ...
        if (currentDrawable != null) {
            if (this.mCurrentMode != 3) {
                if (this.mCurrentMode != 6) {
                    ...
                    int max = Math.max(this.mMinWidth, Math.min(this.mMaxWidth, currentDrawable.getIntrinsicWidth()));
            ...
            int max2 = Math.max(this.mMinWidth, Math.min(this.mMaxWidth, currentDrawable.getIntrinsicHeight()));
            i4 = Math.max(this.mMinHeight, Math.min(this.mMaxHeight, currentDrawable.getIntrinsicWidth()));
            i3 = Math.max(intrinsicHeight2, max2);
        } else {
            i4 = 0;
            i3 = 0;
        }
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        if (action == 0) {
            this.mIsDraggingForSliding = false;
            if (this.mCurrentMode == 5 || this.mCurrentMode == 6 || supportIsInScrollingContainer()) {
            ...
            if (this.mIsDragging) {
                ...
                if (!(this.mCurrentMode == 3 || this.mCurrentMode == 6 || Math.abs(x - this.mTouchDownX) <= ((float) this.mScaledTouchSlop)) || ((this.mCurrentMode == 3 || this.mCurrentMode == 6) && Math.abs(y - this.mTouchDownY) > ((float) this.mScaledTouchSlop))) {
                    startDrag(motionEvent);
                }
            }
        } else if (action == 3) {
    ...
    public void onStopTrackingTouch() {
        ...
        if (this.mIsSeamless && isPressed()) {
            this.mValueAnimator = ValueAnimator.ofInt(super.getProgress(), (int) (((float) Math.round(((float) super.getProgress()) / SCALE_FACTOR)) * SCALE_FACTOR));
    ...
    public boolean performAccessibilityAction(int i, Bundle bundle) {
        ...
        if (i == 4096 || i == 8192) {
            ...
            if (this.mIsSeamless) {
                i2 = Math.round(((float) (getProgress() + max)) * SCALE_FACTOR);
            } else {
                i2 = getProgress() + max;
            }
    ...
    private void initDualOverlapDrawable() {
        if (this.mCurrentMode == 5) {
            this.mOverlapBackground = new SliderDrawable(this, (float) this.mTrackMinWidth, (float) this.mTrackMaxWidth, this.mOverlapNormalProgressColor);
    ...
    public class SliderDrawable extends Drawable {
        ...
        private void initAnimator() {
            ...
            this.mPressedAnimator = ValueAnimator.ofFloat(f, f2);
            ...
            this.mReleasedAnimator = ValueAnimator.ofFloat(f2, f);
    ...
    public class ThumbDrawable extends Drawable {
        ...
        public void initAnimation() {
            this.mThumbPressed = ValueAnimator.ofFloat((float) this.mRadius, 0.0f);
            ...
            this.mThumbReleased = ValueAnimator.ofFloat(0.0f, (float) this.mRadius);
        ...
        public void setTintList(ColorStateList colorStateList) {
            ...
            if (colorStateList != null) {
                this.mColorStateList = colorStateList;
                this.mColor = this.mColorStateList.getColorForState(SeslAbsSeekBar.this.getDrawableState(), this.mColor);
                this.mPaint.setColor(this.mColor);
                invalidateSelf();
            }
        }
